  ...
  return (
    <div className="px-5">
      <div className="my-8">
        ...
        <div className="grid grid-cols-1 grid-rows-1 justify-items-center my-2 pt-5">
      ...
      <table class="min-w-full border-collapse block md:table border-4 border-white">
        <thead class="block md:table-header-group">
          <tr class="border border-grey-500 md:border-none block md:table-row absolute -top-full md:top-auto -left-full md:left-auto  md:relative ">
            <th class="bg-slate-900 p-2 text-white text-center font-bold md:border md:border-grey-500 block md:table-cell">
              No
            </th>
            <th class="bg-slate-900 p-2 text-white text-center font-bold md:border md:border-grey-500 block md:table-cell">
              Nama Ruangan
          ...
          {rooms.map((room, index) => (
            ...
            >
              <td className="p-2 md:border md:border-grey-500 block md:table-cell md:text-center text-left">
                <span className="inline-block w-1/3 md:hidden font-bold mr-3">
                  No
                </span>
                {index + 1}
              </td>
              ...
              <td className="p-2 md:border md:border-grey-500 text-left block md:table-cell items-center justify-center md:text-center">
                ...
                <div className="flex flex-wrap mt-2 justify-center items-center">
                  <Link to={`/admin/rooms/detailroom/${room.id}`}>
                    <button className="bg-green-500 hover:bg-green-700 text-white font-medium py-1 mt-1 px-2 border border-green-500 rounded">
                      Detail
                    </button>
                  </Link>
                  <Link to={`/admin/rooms/${room.id}`}>
                    <button className="bg-blue-500 hover:bg-blue-700 text-white font-medium py-1 mt-1 px-2 border border-blue-500 rounded ml-2">
                      Edit
                    </button>
                  </Link>
                  <button
                    onClick={() => deleteRoom(room.id)}
                    className="bg-red-500 hover:bg-red-700 text-white font-medium py-1 mt-1 px-2 border border-red-500 rounded ml-2"
                  >
                    Delete
                  </button>
                  <button
                    onClick={() => changeStatus(room.id)}
                    className="bg-lime-500 hover:bg-lime-700 text-white font-medium mt-1 py-1 px-2 border border-lime-500 rounded ml-2"
                  >
                    Change Status
                  </button>
                </div>
