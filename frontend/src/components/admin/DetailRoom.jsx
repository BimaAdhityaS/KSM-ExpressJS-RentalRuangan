...
const DetailRoom = () => {
    ...
    return (
        <div>
            <h1 className='text-4xl text-center font-bold mt-5'>Detail Ruangan</h1>
            <hr className='w-full border-0.5 h-0.5 bg-gray-200 mt-2'></hr>
            <section className="text-gray-700 body-font overflow-hidden bg-white">
                <div className="container px-5 py-4 mx-auto">
                    <div className="lg:w-4/5 mx-auto flex flex-wrap">
                        <img alt="ecommerce" className="lg:w-1/2 w-full object-cover object-center rounded border border-gray-200" src={rooms.url}></img>
                        <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
                            <h2 className="text-sm title-font text-gray-500 tracking-widest">Nama Ruangan</h2>
                            <h1 className="text-gray-900 text-3xl title-font font-medium mb-1">{rooms.name}</h1>
                            <div className="flex mb-4">
                                <span className="flex items-center">
                                    <svg fill="currentColor" stroke="currentColor" className="w-6 h-6 text-red-500" viewBox="0 0 32 32">
                                    ...
                                    <p className="text-red-500 ml-3 font-medium">{rooms.kapasitas}</p>
                                </span>
                                <span className="flex ml-3 pl-3 py-2 border-l-2 border-gray-200">
                                    <a className={`${rooms.status === 'CLOSED' ? 'text-red-500 font-medium' : 'text-green-500 font-medium'}`}>
                                        {rooms.status}
                                    </a>
                                </span>
                            </div>
                            ...
                            <p className='mt-4'>
                                <span className='font-medium text-black'>Kontak Pemilik : </span>
                                {rooms.kontak_pemilik}</p>
                            <p className='mt-4'>
                            ...
                            <div className="appearance-none border-gray-400 border-b py-2 focus:outline-none focus:border-red-500 text-base pl-3 pr-10"></div>
                            <div className="flex mt-6">
                                <span className="title-font font-medium text-2xl text-gray-900">{formattedPrice}{rooms.pay_per}</span>
                                <Link className="flex ml-auto text-white bg-blue-500 border-0 py-2 px-6 focus:outline-none hover:bg-blue-600 rounded"  to={`/admin/rooms/${rooms.id}`}>
                                    <button className="flex ml-auto text-white bg-blue-500 border-0 py-2 px-6 focus:outline-none hover:bg-blue-600 rounded">Edit</button>
                                </Link>
                            </div>
                        </div>
                    </div>
